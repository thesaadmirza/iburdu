...
@section('title'){{$user->name}}s concern - @parent @stop
    ...
    <div class="panel-body remove-pd-h">
        @if(count($follows))
            <div class="alert alert-info text-center clearfix">
                @if($currentUser && $currentUser->id == $user->id) my focus @else TA's attention @endif
            </div>
                ...
                @foreach($follows as $follow)
                    <div class="media col-md-4 col-sm-6 bb-15">
                        ...
                        <div class="media-body">
                            ...
                            <div class="text-muted">Join time：{{ $follow->created_at->diffForHumans() }}</div>
                        </div>
                    </div>
                @endforeach
                {!! $follows->render() !!}
