...
@section('container')
    <article class="article" data-id="{{ $article->id }}">
        ...
        <div class="article-footer-meta clearfix">
            <label class="text-heart" data-toggle="tooltip" title="like"><i class="fa fa-{{$currentUser && $article->likes()->byUser($currentUser->id)->count() ? 'thumbs-up' : 'thumbs-o-up'}}"></i> <span>{{$article->like_count}}</span></label>
            ...
            <div class="pull-right">
                <label data-toggle="tooltip" title="Collection"><i class="fa fa-bookmark{{$currentUser && App\Collect::isCollect($currentUser, $article) ? '' : '-o'}}"></i> <span>{{$article->collect_count}}</span></label>
