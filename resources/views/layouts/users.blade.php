            ...
            <div class="col-md-3">
                <div class="ucard">
                    ...
                    <div class="body">
                        ...
                        @if($currentUser && $currentUser->id != $user->id)
                            <div class="">
                                @if(App\User::isFollowing($currentUser, $user))
                                    <span class="label label-warning pointer" onclick="follow(this, {{$user->id}})">unsubscribe</span>
                                @else
                                    <span class="label label-success pointer" onclick="follow(this, {{$user->id}})">attention</span>
                                @endif
                            </div>
                        @endif
                        <hr>
                        <div class="text-muted">
                            <a href="/user/{{ $user->id }}/follows">{{ $user->follows_count }} 关注</a> / <a href="/user/{{ $user->id }}/fans">{{ $user->fans_count }} Fan</a>
                        </div>
                        <hr>
                        <div class="user-meta">
                        ...
                        @can('update', $user)
                            <a href="/user/{{ $user->id }}/edit" class="btn btn-primary btn-sm" data-original-title="" title=""><span class="fa fa-edit"></span> Edit profile</a>
                            <a href="/user/{{ $user->id }}/resetpwd" class="btn btn-warning btn-sm" data-original-title="" title=""><span class="fa fa-edit"></span> change Password</a>
                        @endcan
                    </div>
                </div>
            </div>
